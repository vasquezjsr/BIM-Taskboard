...
namespace SpoolingSavantV3Exports.Workers
{
    ...
    public static class FabricationPartClassification
    {
        ...
        public static bool IsOletPart(FabricationPart part)
        {
            ...
            if (combined.Contains("ANVILET"))
                return true;
            // Butt-weld / socket outlets share olet/anvilet rules (OUTLET ≠ OLET substring — must check explicitly).
            if (combined.Contains("OUTLET"))
                return true;
            if (combined.Contains("OLET") || combined.Contains("WELDOLET") || combined.Contains("THREADOLET"))
                return true;
            return combined.Contains("SOCKOLET");
        }
        ...
        public static bool IsElbowPart(FabricationPart part, Document doc = null)
        {
            ...
            return corpus.Contains("ELBOW") || corpus.Contains(" ELL ") || corpus.Contains(" ELL,") || corpus.EndsWith(" ELL", StringComparison.Ordinal);
        }
