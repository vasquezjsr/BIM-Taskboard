...
namespace SpoolingSavantV3Exports.Workers
{
    ...
    public static class FabricationConnectorEnds
    {
        ...
        /// <summary>True when the connector is a male thread / MPT end.</summary>
        public static bool IsMaleThreadFabricationConnector(Connector connector, Document doc)
        {
            string label = GetConnectorEndLabel(connector, doc);
            if (string.IsNullOrWhiteSpace(label) || label == MissingLabel)
                return false;
            ...
            string up = label.ToUpperInvariant();
            return up.IndexOf("MPT", StringComparison.Ordinal) >= 0
                || up.IndexOf("MNPT", StringComparison.Ordinal) >= 0
                || up.IndexOf("NPT-M", StringComparison.Ordinal) >= 0
                || (up.IndexOf("MALE", StringComparison.Ordinal) >= 0 && up.IndexOf("THREAD", StringComparison.Ordinal) >= 0)
                || (up.IndexOf(" M_", StringComparison.Ordinal) >= 0 && up.IndexOf("MPT", StringComparison.Ordinal) >= 0);
        }

        /// <summary>Threaded or flanged free end — opposite the copper/pipe join on adapters.</summary>
        public static bool IsFreeSideFabricationConnector(Connector connector, Document doc)
        {
            return IsFemaleThreadFabricationConnector(connector, doc)
                || IsMaleThreadFabricationConnector(connector, doc)
                || IsFlangedFabricationConnector(connector, doc);
        }
        ...
        /// <summary>True for weld, groove, sweat, solder, Ftg/hub — pipe-join side of an adapter.</summary>
        public static bool IsPipeJoinFabricationConnector(Connector connector, Document doc)
        {
            ...
            string up = label.ToUpperInvariant();
            if (IsFreeSideFabricationConnector(connector, doc))
                return false;

            return up.IndexOf("WELD", StringComparison.Ordinal) >= 0
                || up.IndexOf("GROOV", StringComparison.Ordinal) >= 0
                || up.IndexOf("SWEAT", StringComparison.Ordinal) >= 0
                || up.IndexOf("SOLDER", StringComparison.Ordinal) >= 0
                || up.IndexOf("BRAZ", StringComparison.Ordinal) >= 0
                || up.IndexOf("FTG", StringComparison.Ordinal) >= 0
                || up.IndexOf("HUB", StringComparison.Ordinal) >= 0
                || up.IndexOf("COPPER", StringComparison.Ordinal) >= 0
                || up.IndexOf("COUPLING", StringComparison.Ordinal) >= 0
                || up.IndexOf("M_C-", StringComparison.Ordinal) >= 0
                || up.IndexOf("M_C_", StringComparison.Ordinal) >= 0;
        }
